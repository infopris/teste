			...
			Else
						...
						SF2->F2_DOC == (cAliasSE1)->E1_NUM     
							If (cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. (cAliasSE1)->E1_TIPO = 'DP' .OR. ((Alltrim((cAliasSE1)->E1_ORIGEM) $ 'LOJA701|FATA701|LOJA010') .AND. (cAliasSE1)->E1_TIPO $ cWhere)
								...
								If cValLiqB == "2" // 1 Baixa - 2 Emissao									
										//Neste caso não subtraio o valor do INSS do titulo. //DSERTSS1-9986
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de operação da nf sobre cupom
						...
						If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
							...
							// Luiz Eduardo - 28/09/2017 - Mensagem da Nota buscar na tabela SC5
							ElseIf !(IIF( SC5->(FieldPos("C5_MENNOTA")) > 0, AllTrim(SC5->C5_MENNOTA),AllTrim(SC5->C5_MENNOTA)) $ cMensCli)
...
//							EndIf
						...
						EndIf
						// Luiz - 29/06/2020
						If !Empty(SC6->C6_PEDCLI) .And. !AllTrim(FORMULA(SC6->C6_PEDCLI)) $ cMensCli
							cMensCli += " PEDIDO DO CLIENTE : "+AllTrim(SC6->C6_PEDCLI)
						EndIf
						...
						EndIf
					// Luiz - 28/03/2018
					// VERIFICA SE MENSAGEM PADRÃO TEM SEQUENCIA (AMIGAO)
					if SC5->C5_MENPAD >='32'
						Select SM4
						seek xFilial()+left(SC5->C5_MENPAD,2)+"A"
						If !eof() .And. !AllTrim(FORMULA(SM4->M4_CODIGO)) $ cMensFis
							cMensFis += AllTrim(FORMULA(SM4->M4_CODIGO))
						EndIf
						seek xFilial()+left(SC5->C5_MENPAD,2)+"B"
						If !eof() .And. !AllTrim(FORMULA(SM4->M4_CODIGO)) $ cMensFis
							cMensFis += AllTrim(FORMULA(SM4->M4_CODIGO))
						EndIf
					Endif
					// Luiz - 25/06/2012 
					If !Empty(SC5->C5_MENPAD1) .And. !AllTrim(FORMULA(SC5->C5_MENPAD1)) $ cMensFis
						cMensFis += AllTrim(FORMULA(SC5->C5_MENPAD1))
					EndIf
					// Luiz - 27/12/2016 - Redespacho
					If !Empty(SC5->C5_REDESP) 
						Select SA4
						nReg := recno()
						Seek xFilial()+SC5->C5_REDESP
						cRedesp := " REDESPACHO: "+SA4->A4_NOME+" END."+AllTrim(SA4->A4_END)+" "+AllTrim(SA4->A4_BAIRRO)+" - "+SA4->A4_MUN+"/"+SA4->A4_EST+" CNPJ:"+SA4->A4_CGC
						if !AllTrim(cRedesp) $ cMensFis
							cMensFis += (cRedesp)
						Endif
						goto nReg
					EndIf // Final Msg.Redespacho
					// Luiz - 08/06/2020 - Endereço de Cobrança
					Select SUA
					dbSetOrder(1)
					Seek xFilial()+SC5->C5_NUMTMK
					If !Empty(SUA->UA_ENDENT) .and. trim(SUA->UA_ENDENT)<>trim(sa1->a1_End)
						if !AllTrim(SUA->UA_ENDENT) $ cMensFis 
							cMensFis += " - Local de entrega "+AllTrim(SUA->UA_ENDENT)+"  "+AllTrim(SUA->UA_BAIRROE)+"  "+AllTrim(SUA->UA_MUNE) + " / " + SUA->UA_ESTE
						endif
					EndIf // Final Msg.End.Cobrança
						...
						ElseIf !Empty(SF4->F4_FORMULA) .And. Formula(SF4->F4_FORMULA) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(Formula(SF4->F4_FORMULA)) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(Formula(SF4->F4_FORMULA))$cMensFis) )
							...
							EndIf
						EndIf
					If !AllTrim(FORMULA(SF4->F4_FORMULA)) $ cMensFis // Luiz Eduardo
						If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
							cMensFis += " "
						EndIf
					...
					EndIf
						...
						// Alteração para buscar CEST apenas quando F4_SITTRIB ='010' - Luiz Eduardo 06/07/2017
						cCest := ""
						if SF4->F4_SITTRIB ='10'
							do case
							Case sb1->b1_posipi = '70099200' .or. sb1->b1_posipi = '70099100'
								cCest := '1008000'
							Case sb1->b1_posipi = '70052900' 
								cCest := '1003500'
							endcase
						endif
						// Final da alteração
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							cCest,; //Alteração Luiz 06/07/2017 //IIF(SB1->(FieldPos("B1_CEST"))<>0,SB1->B1_CEST,""),; //aProd[41] NT2015/003
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			aadd(aNota,iif(val(SF1->F1_HORA)<>0,SF1->F1_HORA,time())) // Luiz - 09-03-2016 para quando o sistema não trouxer  hora da emissão
					...
					If !(cAliasSD1)->D1_TIPO $ "DBN" .Or. lRural
						...
						if !lRural .and. lSeekOk
							...
							if cChave <> dToS( SD1->D1_EMISSAO ) + SD1->D1_SERIE + SD1->D1_DOC + iIf( SD1->D1_TIPO $ "DB", iIf( SD1->D1_FORMUL == "S", SM0->M0_CGC, SA1->A1_CGC ), iIf( SD1->D1_FORMUL == "S", SM0->M0_CGC, SA2->A2_CGC ) ) + SM0->M0_ESTCOB + SF1->F1_ESPECIE + SF1->F1_CHVNFE;
								...
								EndIf
						else // Luiz - NFe Vinculada (Devolução)
...
// Desconta Zona Franca - Abati apenas d1_desczfr e inclui o d1_desczfr ao invés do d1_desczfp e d1_deczpc								            		            										
				aadd(aProd,	{Len(aProd)+1,;  
					cCodProd,;
					IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
					cDescProd,;
					...
					SB1->B1_UM,;
					(cAliasSD1)->D1_QUANT,;
					IIF(!((cAliasSD1)->D1_TIPO $"IP" .Or. ((cAliasSD1)->D1_TIPO $ "D" .And. cTpOrig == "P")) ,(IIF(!(lMvNFLeiZF),(cAliasSD1)->D1_TOTAL,(cAliasSD1)->D1_TOTAL - ((cAliasSD1)->D1_DESCZFR+(cAliasSD1)->D1_DESCZFP+(cAliasSD1)->D1_DESCZFC))),0),;
					...
					IIF((cAliasSD1)->(FieldPos("D1_DESCZFR"))<>0,(cAliasSD1)->D1_DESCZFR,0),;			//Desconto Zona Franca PIS
					0,; //IIF((cAliasSD1)->(FieldPos("D1_DESCZFC"))<>0,(cAliasSD1)->D1_DESCZFC+(cAliasSD1)->D1_DESCZFR,0),;			//Desconto Zona Franca CONFINS
...
If !Empty(aNota)
	...
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc,aProd,aDI,aNfVincRur,aRetido,cNfRefcup,cSerRefcup,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,Iif(aNota[5] == "D",aRefECF,{}),nVicmsDeson,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,nvBCUFDest,aICMUFDest,nValIpiBene,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aObsCont,aValTotOpe,cMensDifal,aProcRef,aDest,nTotCrdP)
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet,cNfRefcup,cSerRefcup,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,aRefECF,nVicmsDeson,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,nvBCUFDest,aICMUFDest,nValIpiBene,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aObsCont,aValTotOpe,cMensDifal,aProcRef,aDest,nTotCrdP)
Local cString   		:= ""
Local cCfor     		:= ""
Local cLojaEn   		:= ""
Local cCnpjen   		:= ""
...
Do Case
	...
	Case (cVeramb >= "3.10" .and. (!Empty(aNfVinc) .Or. !Empty(aRefECF) .Or. !Empty(aNfVincRur))) .and. ( (aNota[5] $ "D|B" .And. SF4->F4_AJUSTE <> "S") .or. (aNota[5] $ "N" .And. SF4->F4_PODER3 == "D") )
     ...
     documento fiscal referenciado conforme  NT 2013/005 v 1.20.
   	 */
    Case (aNota[5]) $ "D" .and. Empty(aNfVinc).and. Alltrim(SFT->FT_CFOP) $ "1201-1202-1410-1411-5921-6921-2201-2202-2203" // Luiz- adicionei os CFOPs -2201-2202 09-03-2016"
