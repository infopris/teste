...
#DEFINE MAXITEMC   045                                                // Máxima de caracteres por linha de produtos/serviços
...
ElseIf nTipo == 1
	...
	If lPergunte
		RPTStatus( {|lEnd| U_DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja, nTipo)}, "Imprimindo DANFE..." )
	EndIf
EndIf
				...
				If !Empty(aXML[nX][2])
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						If Empty(cAviso) .And. Empty(cErro)
							ImpDet(@oDanfe,oNFe,cAutoriza,cModalidade,oNfeDPEC,cCodAutDPEC,aXml[nX][6],aXml[nX][7],aNotas[nX],lImpSimp,nTipo)
							lExistNfe := .T.
						EndIf
						oNfe     := nil
						oNfeDPEC := nil
...
if Len(cString)=11
	nMax :=11 		// Espelhos Leão - retirar quebra do campo código
endif
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	if !oPrinter:lInJob
		...
		aTamCol := {50, 150, 40, 16, 24, 14, 35, 53, 38, 36, 30, 20, 31, 24}
	endif
	...
	If nAux > 603
		aTamCol[2] -= nAux - 603
		// Espelhos Leão - retirar quebra do campo código
		aTamCol[8] := 43
		aTamCol[1] := 60
	EndIf
