...
If lBackup
    If !Empty(cPerIni) .And. !Empty(cUmTFerM)
        ...
        SRR->( dbSetOrder(1) )
        //Ordena a tabela SRD pela ordem 1 - RD_FILIAL+RD_MAT+RD_DATARQ+RD_PD+RD_SEMANA+RD_SEQ+RD_CC+RD_PROCES
        SRD->( dbSetOrder(1) )
        ...
        While (cAliasQry)->( !EoF() )
            ...
            If nVFerias > 0
                ...
                If SRR->( RecLock("SRR", lNovo) )
                    ...
                    SRR->( MsUnlock() )
                EndIf
                lNovo := SRD->( !dbSeek( SRH->RH_FILIAL + SRH->RH_MAT + AnoMes(SRH->RH_DATAINI) + cUmTFerM + SRH->RH_NPAGTO + cSeqFer + cCCFer) )
                //Trava o registro na SRD para edição
            ...
            If nVFeriasMS > 0
                ...
                If SRR->( RecLock("SRR", lNovo) )
                    ...
                    SRR->( MsUnlock() )
                EndIf
                lNovo := SRD->( !dbSeek( SRH->RH_FILIAL + SRH->RH_MAT + AnoMes(SRH->RH_DATAINI) + cUmTFerMs + SRH->RH_NPAGTO + cSeqFerMs + cCCFerMs) )
                //Trava o registro na SRD para edição
                If SRD->( RecLock("SRD", lNovo) )
                    //Se for inclusão, grava todos campos da srd
                    //Se for alteração, apenas altera o valor do registro
            ...
            If nValAbono > 0
                ...
                If SRR->( RecLock("SRR", lNovo) )
                    ...
                    SRR->( MsUnlock() )
                EndIf
                lNovo := SRD->( !dbSeek( SRH->RH_FILIAL + SRH->RH_MAT + AnoMes(SRH->RH_DATAINI) + cUmTAboM + SRH->RH_NPAGTO + cSeqAbo + cCCAbo) )
                //Trava o registro na SRD para edição
                If SRD->( RecLock("SRD", lNovo) )
                    //Se for inclusão, grava todos campos da srd
                    //Se for alteração, apenas altera o valor do registro
            ...
            If nValAbonoMs > 0
                ...
                If SRR->( RecLock("SRR", lNovo) )
                    ...
                    SRR->( MsUnlock() )
                EndIf
                lNovo := SRD->( !dbSeek( SRH->RH_FILIAL + SRH->RH_MAT + AnoMes(SRH->RH_DATAINI) + cUmTAboMs + SRH->RH_NPAGTO + cSeqAboMs + cCCAboMs) )
                //Trava o registro na SRD para edição
                If SRD->( RecLock("SRD", lNovo) )
                    //Se for inclusão, grava todos campos da srd
                    //Se for alteração, apenas altera o valor do registro
