...
±±³Descri‡…o ³ PLANO DE MELHORIA CONTINUA        ³Programa     MATR460.PRX³±±
...
If !(FindFunction("SIGACUS_V")	.And. SIGACUS_V() >= 20060810)
    Final("" + " SIGACUS.PRW !!!") // "Atualizar SIGACUS.PRW"
EndIf
If !(FindFunction("SIGACUSA_V")	.And. SIGACUSA_V() >= 20060321)
    Final("" + " SIGACUSA.PRX !!!") // "Atualizar SIGACUSA.PRX"
EndIf
...
//³                                                                        ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oReport:= TReport():New("MATR460","","MTR460", {|oReport| ReportPrint(oReport)},"") //"Registro de Invent rio - Modelo P7"##"Emiss„o do Registro de Invent rio.Os Valores Totais serao impressos conforme Modelo Legal"
...
oSection1 := TRSection():New(oReport,"",{"SB1"}) //"Saldos em Estoque"
			...
			If lLivro
				...
				If lSaldTerceir
					#IFNDEF TOP
						...
						IndRegua("SB6",cIndSB6,cChave,,cQuery,"")		//"Selecionando Poder Terceiros..."
				...
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ			
				If lSaldProcess
					R460EmProcesso(@lEnd,cArqTemp,.T.,cProdIni,cProdFim,cAlmoxIni,cAlmoxFim,mv_par20==1,dDtFech,lCalcProcDt,nQuebraAliq,lListCustMed)
				EndIf
				...
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				cSitAnt	:="X"
				aSituacao:={" EM ESTOQUE "," EM PROCESSO "," SEM MOVIMENTACAO "," DE TERCEIROS "," EM TERCEIROS "} //STR0015,STR0016,STR0017,STR0018,STR0019,STR0034}		//" EM ESTOQUE "###" EM PROCESSO "###" SEM MOVIMENTACAO "###" DE TERCEIROS "###" EM TERCEIROS "
				cTipoAnt:="XX"
				cQuebra := ""
					...
					While !oReport:Cancel() .And. !Eof() .And. cSitAnt == SITUACAO
						...
						While !oReport:Cancel() .And. !Eof() .And. cSitAnt+cTipoAnt == SITUACAO+TIPO
							...
							While !oReport:Cancel() .And. !Eof() .And. cQuebra==&(cKeyQbr)
								...
								If nQuebraAliq <> 1
									If lImpAliq
										FmtLinR4(oReport,{"",Padc(" "+""+Transform(nAliq,"@E 99.99%")+" ",35,"*"),"","","","",""},aL[15],,,@nLin)
										FmtLinR4(oReport,Array(7),aL[15],,,@nLin)
										lImpAliq := .F.
									EndIf	
								EndIf	
								...
								If nQuebraAliq <> 1 .And. cQuebra <> &(cKeyQbr)
									...
									FmtLinR4(oReport,{,""+""+Transform(nAliq,"@E 99.99%")+" ===>",,,,,Transform(aTotal[nPos,5], "@E 999,999,999,999.99")},aL[15],,,@nLin)			//"TOTAL "
									FmtLinR4(oReport,Array(7),aL[15],,,@nLin)
								EndIf
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function MATR460R3()
Local wnrel
Local Titulo	:= "Registro de Invent rio - Modelo P7"
Local cDesc1	:= "Emiss„o do Registro de Invent rio.Os Valores Totais serao impressos conforme Modelo Legal"
Local cDesc2	:= ""
Local cDesc3	:= ""
Local cString	:= "SB1"
		...
		If !Empty(aFilsCalc)
			...
			For nForFilial := 1 To Len( aFilsCalc )
				...
				If aFilsCalc[ nForFilial, 1 ]
					...
					If lLivro
						RptStatus( { |lEnd| R460Imp(@lEnd,wnRel,cString,Tamanho,aFilsCalc[ nForFilial,3])}, Titulo  , "" + aFilsCalc[ nForFilial,2 ] + " - " + aFilsCalc[ nForFilial,3])
					Else
						RptStatus( { |lEnd| R460Term(@lEnd,wnRel,cString,Tamanho) }, Titulo, "" + aFilsCalc[ nForFilial,2 ] + " - " + aFilsCalc[ nForFilial,3])
					EndIf
...
#IFDEF TOP
	...
	MsAguarde({|| dbUseArea(.T.,"TOPCONN",TCGenQry(,,cQuery),cAliasTop,.F.,.T.)},"")
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ			
If lSaldProcess
	R460EmProcesso(@lEnd,cArqTemp,Nil,cProdIni,cProdFim,cAlmoxIni,cAlmoxFim,mv_par20==1,dDtFech,lCalcProcDt,nQuebraAliq,lListCustMed)
EndIf
	...
	While !Eof() .And. cSitAnt == SITUACAO
		...
		While !Eof() .And. cSitAnt+cTipoAnt == SITUACAO+TIPO
			...
			While !Eof() .And. cQuebra==&(cKeyQbr)
				...
				If nQuebraAliq <> 1
					If lImpAliq
						FmtLin({"",Padc(" "+""+Transform(nAliq,"@E 99.99%")+" ",35,"*"),"","","","",""},aL[15],,,@nLin)
						FmtLin(Array(7),aL[15],,,@nLin)
						lImpAliq := .F.
					EndIf	
				EndIf	
				...
				If nQuebraAliq <> 1 .And. cQuebra <> &(cKeyQbr)
					FmtLin(Array(7),aL[15],,,@nLin)
					nPos:=Ascan(aTotal,{|x|x[1]==cSitAnt.And.x[2]==cTipoAnt.And.x[6]==nAliq})
					FmtLin({,""+""+Transform(nAliq,"@E 99.99%")+" ===>",,,,,Transform(aTotal[nPos,5], "@E 999,999,999,999.99")},aL[15],,,@nLin)			//"TOTAL "
					FmtLin(Array(7),aL[15],,,@nLin)
				EndIf
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function R460EmProcesso(lEnd,cArqTemp,lGraph, cProdIni, cProdFim, cAlmoxIni, cAlmoxFim, lModProces, dDtFech, lCalcProcDt, nQuebraAliq, lListCustMed)
Local aCampos   := {}
Local aEmAnalise:= {}
Local aSalAtu   := {}
Local aProducao := {}
...
If cSituacao!="T"
	...
	If cTipo!=TT
		...
		If nQuebraAliq == 1
			...
			Else
				FmtLinR4(oReport,{,"Total"+TRIM(X5Descri())+" ===>",,,,,Transform(aTotal[nPos,5], "@E 999,999,999,999.99")},aL[15],,,@nLin)			//"TOTAL "
			EndIf	
		Else
			nTotal:=0
			...
			EndDo
			If !lGraph
				FmtLin({,"Total"+TRIM(X5Descri())+" ===>",,,,,Transform(nTotal, "@E 999,999,999,999.99")},aL[15],,,@nLin)			//"TOTAL "
			Else
				FmtLinR4(oReport,{,"Total"+TRIM(X5Descri())+" ===>",,,,,Transform(nTotal, "@E 999,999,999,999.99")},aL[15],,,@nLin)			//"TOTAL "
			EndIf	
		EndIf	
	Else
...
Else
	...
	If !lGraph
		FmtLin(Array(7),aL[15],,,@nLin)
		FmtLin({,"Total",,,,,},aL[15],,,@nLin)		//"R E S U M O"
		FmtLin({,"***********",,,,,},aL[15],,,@nLin)
	Else
		FmtLinR4(oReport,Array(7),aL[15],,,@nLin)
		FmtLinR4(oReport,{,"Total",,,,,},aL[15],,,@nLin)		//"R E S U M O"
	...
	For i:=1 to Len(aTotal)
		...
		If cSitAnt!=aTotal[i,1]
			...
			Else
				If !lGraph
					FmtLin(Array(7),aL[15],,,@nLin)
					FmtLin({,"Total",,,,,Transform(aTotal[i,5], "@E 999,999,999,999.99")},aL[15],,,@nLin)		//"TOTAL GERAL ====>"
				Else
					FmtLinR4(oReport,Array(7),aL[15],,,@nLin)
					FmtLinR4(oReport,{,"Total",,,,,Transform(aTotal[i,5], "@E 999,999,999,999.99")},aL[15],,,@nLin)		//"TOTAL GERAL ====>"
				EndIf	
			EndIf
		EndIf
		If aTotal[i,1]!="T"
			If aTotal[i,2]!=TT
				...
				If nQuebraAliq <> 1	
					If !lGraph
						FmtLin({,"Total"+Transform(aTotal[i,6],"@E 99.99%"),,,,,Transform(aTotal[i,5], "@E 999,999,999,999.99")},aL[15],,,@nLin)			
					Else
						FmtLinR4(oReport,{,"Total"+Transform(aTotal[i,6],"@E 99.99%"),,,,,Transform(aTotal[i,5], "@E 999,999,999,999.99")},aL[15],,,@nLin)			
					EndIf	
				EndIf
			Else
				If !lGraph
...
If !lGraph
	FmtLin(Array(7),aL[15],,,@nLin)
	FmtLin({,"",,,,,},aL[15],,,@nLin) //"ESTOQUE INEXISTENTE"
Else
	FmtLinR4(oReport,Array(7),aL[15],,,@nLin)
	FmtLinR4(oReport,{,"",,,,,},aL[15],,,@nLin) //"ESTOQUE INEXISTENTE"
EndIf	
Return Nil
...
If dbSeek(PADR("MTR460",nTamSX1)+"02",.F.)
	If Empty(X1_DEF04)
		...
		MsUnlock()
		aHelpPor := {'Considera o saldo em poder de terceiro',;
		             ' na filtragem dos dados.              ',;
	    	         '' }
		aHelpSpa :=	{'Considera el saldo en poder de tercero',;
		             ' en el filtrado de datos.             ',;
	    	         '' }
		aHelpEng := {'Considers the balance in third party  ',;
		             'possession when filtering data.       ',;
	    	         '' }
		PutSX1Help("P.MTR46002.",aHelpPor,aHelpEng,aHelpSpa)
	EndIf           
EndIf
...
If GetMv("MV_IMPSX1") == "S" .And. Substr(cAcesso,101,1) == "S"   // Imprime pergunta no cabecalho
	If m_pag == 1
		...
		If !Empty(cFiltro)
			nLin += 2
			@ nLin,5  PSAY OemToAnsi("") + Substr(cFiltro,nCont,nLargura-19)  // "Filtro      : "
